...
const durationOptions = [20, 30, 60, 90, 120]
...
const totalDuration = computed(() => {
  ...
  const [startH, startM] = reservationForm.value.startTime.split(':').map(Number)
  const [endH, endM] = reservationForm.value.endTime.split(':').map(Number)
...
// Overall time validation
const isTimeValid = computed(() => {
  if (!reservationForm.value.startTime || !reservationForm.value.endTime) return false
  if (totalDuration.value <= 0) return false
  if (!selectedField.value) return false
  
  return isDurationValid.value && isWithinFieldHours.value
})
...
const simulateConflict = () => {
  ...
  if (fieldAvailability.value[randomField.id][randomDay] === 'free') {
    // Add flashing class
    flashingCells.value.add(cellKey)
    
    // Update availability to occupied
    fieldAvailability.value[randomField.id][randomDay] = 'occupied'
...
let conflictInterval: ReturnType<typeof setInterval> | null = null
...
<template>
  <div class="sports-page-root" :class="mdAndDown ? 'pa-0' : 'pa-1'">
    ...
    <div v-if="currentView !== 'availability'" class="page-header-simple" :class="[mdAndDown ? 'pa-2' : 'pa-4']" style="margin-top: -8px;">
      <div class="d-flex align-center">
        ...
        <div>
          <h1 class="text-h6 text-sm-h5 font-weight-bold" style="white-space: normal; line-height: 1.2;">{{ pageInfo.title }}</h1>
    ...
    <template v-if="currentView === 'balance'">
              ...
              <div class="pa-6 balance-load-section">
                ...
                <div class="d-flex flex-wrap gap-2 mb-4">
                  <v-chip 
                    ...
                    :class="loadAmount === amount ? 'text-deep-orange' : 'text-white'"
                    size="large"
      ...
      <v-alert type="info" variant="tonal" class="mb-4" density="compact" border="start">
        ...
        <span class="text-body-2">Yüklediğiniz tutarların geri iadesi için <strong>Sağlık Kültür ve Spor Daire Başkanlığı</strong>'na başvurmanız gerekmektedir.</span>
      </v-alert>
            ...
            >
              <v-card-text class="d-flex align-center pa-3">
                <v-avatar 
                  ...
                  class="mr-3"
                >
                  <v-icon color="white">{{ tx.type === 'Bakiye Yükleme' ? 'mdi-arrow-down' : tx.type === 'Bakiye İadesi' ? 'mdi-undo' : 'mdi-arrow-up' }}</v-icon>
                </v-avatar>
                ...
                <div class="text-right">
                  ...
                  >
                    {{ tx.type === 'Bakiye Yükleme' || tx.type === 'Bakiye İadesi' ? '+' : '-' }}{{ tx.amount }} ₺
                  </div>
                  <v-chip v-if="tx.status" color="success" size="x-small" variant="flat">{{ tx.status }}</v-chip>
                </div>
              </v-card-text>
            </v-card>
          </div>
        ...
        <v-table v-else density="comfortable" class="transactions-table">
          ...
          <tbody>
            <tr v-for="tx in transactions" :key="tx.id" class="transaction-row">
              <td>
                <div class="d-flex align-center">
                  <v-avatar 
                    ...
                    class="mr-2"
                  >
                    <v-icon size="16" color="white">{{ tx.type === 'Bakiye Yükleme' ? 'mdi-arrow-down' : tx.type === 'Bakiye İadesi' ? 'mdi-undo' : 'mdi-arrow-up' }}</v-icon>
                  </v-avatar>
                  {{ tx.type }}
                </div>
              </td>
              ...
              <td>
                <v-chip v-if="tx.status" color="success" size="small" variant="flat">{{ tx.status }}</v-chip>
        ...
        >
          <v-card-text class="pa-3">
            ...
            <div class="d-flex align-center justify-space-between">
              ...
              <v-btn color="deep-orange" size="default" variant="tonal" prepend-icon="mdi-calendar-plus" @click="openReservationDialog(field)">
                Rezervasyon
              </v-btn>
            </div>
          </v-card-text>
    ...
    <template v-if="currentView === 'availability'">
      ...
      <v-card class="content-card sticky-card mt-n2" elevation="0">
        <v-card-title class="d-flex flex-column flex-sm-row align-start align-sm-center justify-space-between px-3 border-bottom gap-2" style="padding-top: 8px; padding-bottom: 8px;">
          <div class="d-flex align-center">
            ...
            <thead>
              <tr>
                ...
                <th v-for="(day, idx) in weekDays" :key="idx" class="heatmap-header-cell day-cell">
                  ...
                  <div class="text-caption text-medium-emphasis">{{ weekDates[idx].getDate() }}/{{ weekDates[idx].getMonth() + 1 }}</div>
                </th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="field in heatmapFields" :key="field.id" class="heatmap-row">
                <td class="field-name-cell">
                  <div class="field-name-content">
                    <v-avatar :color="'deep-orange-lighten-4'" size="20" class="mr-1 flex-shrink-0">
                      <v-icon :icon="getFieldIcon(field.type)" color="deep-orange" size="12" />
                    </v-avatar>
                    <div class="field-name-text">
                      <div class="font-weight-medium">{{ field.name }}</div>
                      <div class="text-caption text-medium-emphasis d-none d-md-block">{{ field.type }}</div>
                    </div>
                  </div>
                </td>
                <td 
                  v-for="(status, dayIdx) in fieldAvailability[field.id]" 
                  :key="dayIdx" 
                  class="heatmap-cell"
                ...
                >
                  <v-tooltip :text="status === 'free' ? 'Rezervasyon için tıkla' : status === 'occupied' ? 'Dolu' : 'Bakımda'" location="top">
                    <template #activator="{ props }">
                      <div v-bind="props" class="heatmap-cell-inner">
                        <v-icon v-if="status === 'maintenance'" size="16" color="grey-darken-1">mdi-wrench</v-icon>
                      </div>
                    </template>
                  </v-tooltip>
                </td>
          ...
          <v-row dense>
            <v-col cols="6" sm="3">
              <div class="text-center">
                <div class="text-h5 font-weight-bold text-success">{{ heatmapFields.length * 7 - 20 }}</div>
                <div class="text-caption">Müsait Slot</div>
              </div>
            </v-col>
            <v-col cols="6" sm="3">
              <div class="text-center">
                <div class="text-h5 font-weight-bold text-amber-darken-2">{{ reservations.filter(r => !r.cancelled).length }}</div>
        ...
        <v-col cols="6" md="3">
          <v-card class="stats-card pa-4" variant="outlined">
            <div class="d-flex align-center">
              ...
              <div>
                <div class="text-h5 font-weight-bold">{{ reservations.filter(r => r.approved && !r.cancelled).length }}</div>
        ...
        <v-col cols="6" md="3">
          <v-card class="stats-card pa-4" variant="outlined">
            <div class="d-flex align-center">
              ...
              <div>
                <div class="text-h5 font-weight-bold">{{ reservations.filter(r => r.cancelled).length }}</div>
        ...
        <v-col cols="6" md="3">
          <v-card class="stats-card pa-4" variant="outlined">
            <div class="d-flex align-center">
              ...
              <div>
                <div class="text-h5 font-weight-bold">{{ reservations.filter(r => !r.cancelled).reduce((sum, r) => sum + (r.price || 0), 0) }} ₺</div>
      ...
      <v-card class="content-card mb-4" elevation="0">
        <v-card-text class="py-3">
          ...
          <v-row dense align="center">
            ...
            <v-col cols="6" sm="3" md="2">
              <v-text-field
                v-model="reservationFilterDateStart"
                label="Başlangıç"
                type="date"
                variant="outlined"
                density="compact"
            ...
            <v-col cols="6" sm="3" md="2">
              <v-text-field
                v-model="reservationFilterDateEnd"
                label="Bitiş"
                type="date"
                variant="outlined"
                density="compact"
        ...
        >
          <v-card-text class="pa-4">
            <div class="d-flex justify-space-between align-start mb-3">
              ...
              <v-chip 
                :color="res.cancelled ? 'error' : (res.approved ? 'success' : 'warning')" 
                size="small" 
                :variant="res.cancelled ? 'flat' : 'tonal'"
              >
                {{ res.status }}
              </v-chip>
            </div>
            ...
            <v-row dense class="text-body-2">
              ...
              <v-col cols="6" class="mt-2">
                ...
                <div class="font-weight-medium">{{ res.paymentMethod === 'card' ? 'Kart' : 'Bakiye' }}</div>
              </v-col>
            </v-row>
    ...
    <v-card>
      ...
      <v-card-text v-if="selectedField">
        ...
        <v-card v-if="totalDuration > 0" variant="outlined" class="mb-4 pa-3">
          ...
          <v-alert v-if="!isDurationValid && totalDuration > 0" type="warning" variant="tonal" density="compact" class="mt-2">
            <v-icon size="small" class="mr-1">mdi-alert</v-icon>
            Süre {{ selectedField.defaultDuration }} dakikanın katı olmalıdır!
          </v-alert>
          
          <!-- Field hours validation error -->
          <v-alert v-if="!isWithinFieldHours && totalDuration > 0" type="error" variant="tonal" density="compact" class="mt-2">
            <v-icon size="small" class="mr-1">mdi-clock-alert</v-icon>
            Seçilen saatler saha çalışma saatleri ({{ selectedField.startTime }} - {{ selectedField.endTime }}) dışında!
          </v-alert>
        </v-card>
        ...
        <v-card variant="outlined" class="mt-4 pa-3">
          <a :href="getFieldScheduleImage(selectedField.id)" target="_blank" class="schedule-image-link-inline">
            ...
            <div>
              ...
              <div class="text-caption text-medium-emphasis">Bu sahanın detaylı programını görüntüleyin</div>
            </div>
        ...
        <div class="d-flex justify-space-between align-center">
          ...
          <div v-if="reservationForm.paymentMethod === 'balance'">
            ...
            <div class="text-h6" :class="isBalanceSufficient ? 'text-success' : 'text-error'">{{ balance }} ₺</div>
          </div>
        </div>

        <v-alert v-if="reservationForm.paymentMethod === 'balance' && !isBalanceSufficient" type="error" variant="tonal" density="compact" class="mt-4">
...
.heatmap-scroll-container {
  overflow-x: auto;
  overflow-y: visible; /* Critical: allows sticky to work with page scroll */
...
.heatmap-header-cell {
  padding: 6px 4px; /* Balanced padding for readability */
  text-align: center;
  background: rgb(var(--v-theme-surface)) !important; /* Solid background for sticky */
}
...
@media (max-width: 600px) {
  ...
  .heatmap-table {
    border-spacing: 4px;
    font-size: 12px;
    min-width: 500px; /* Force horizontal scroll instead of squeezing */
  }
...
.bg-success-lighten-4 { background-color: rgba(76, 175, 80, 0.3) !important; }
.bg-error-lighten-4 { background-color: rgba(244, 67, 54, 0.3) !important; }
.bg-grey-lighten-2 { background-color: rgba(189, 189, 189, 0.4) !important; }
